...
1. **必ず日本語で応答する**
2. **コードを変更する前に「何を・なぜ変えるのか」をユーザーに説明し、許可を得てから実行する**
...
## 2026-05-24 実装・修正した内容

### 共通コンテンツ（定型文）の挿入位置を3箇所に整理

変更前：6箇所（文頭WP・画像前・画像後・文末WP・文末Instagram・文末Threads）
変更後：3箇所に統一

1. **文頭（共通・全媒体）**
   - WordPress・Instagram・Threads すべてに適用
   - WordPress：`</h1>`タグの直後に挿入（タイトルの次）
   - Instagram：キャプション1行目（フック文）の後に挿入
   - Threads：本文の前に挿入

2. **画像前（WordPressのみ）**
   - WordPressの本文末尾・画像の直前に挿入
   - SNSには適用しない

3. **文末（SNSのみ）**
   - Instagram・Threads 共通で本文の後に挿入
   - WordPress には適用しない

グローバル設定とサロン別設定の両方で同じ3項目を表示。

### Instagramキャプションの投稿順序

```
タイトル行（Geminiが生成するフック文）
文頭定型文
本文続き...
文末定型文
#ハッシュタグ（最後）
...
変更前は定型文がハッシュタグの後に来たり、Threadsで文頭が末尾に出る問題があったが修正済み。

### ハッシュタグの扱い

- 変更前：即時投稿ではハッシュタグがキャプションに含まれていなかった（バグ）。スケジュール投稿のみ server.ts でキャプションに結合していた
- 変更後：即時投稿でもハッシュタグをキャプションに含めて投稿するよう修正。スケジュール投稿も区切りを `\n` から `\n\n` に統一

### Webプレビュー機能

- 記事一覧の各記事にWebプレビューボタンを追加
- iframe（srcDoc）でHTMLをレンダリング。定型文込みの見た目を確認できる
- タイトルが二重表示されていたバグを修正（Geminiの本文に`<h1>`が含まれているため、`<h1>`を別途追加しないようにした）

### SNSプレビューの改善

- 記事一覧のSNS Previewエリアに定型文・ハッシュタグ込みのキャプションを表示（以前は生のGemini出力のみ）
- 詳細編集画面のInstagram・Threadsテキストエリアの下に「投稿時のプレビュー（定型文込み）」を追加。定型文が設定されている場合のみ表示する

### Geminiプロンプトから削除したもの

「詳細はプロフィールのリンクからブログをチェック！」というCTAの指示を、記事生成プロンプトとSNS再生成プロンプトの2箇所から削除。これ以降は自動でこのフレーズが入らなくなった。

### バッチモードエラー表示修正

バッチモード（`isBatchMode=true`）で `generateBlogPost` 内のエラーが起きると、`if (!isBatchMode)` の条件でエラーが隠されていた。バッチモード中でもエラーが表示されるよう修正済み。

---

## 2026-05-25 実装・修正した内容

### loop_interval_days フィールドは「分」単位（重要）

DBの `loop_interval_days` カラムの値は実際には**分単位**。
...
計算式：`loop_interval_days * 60 * 1000`（ミリ秒変換）

**絶対に `* 24 * 60 * 60 * 1000` にしない。** 以前この変換式に変更してバグを起こした。server.ts と supabase/functions/process-scheduled-posts/index.ts の両方で同じ計算式を使う。

### postingModalの仕組み

「今すぐ投稿」「予約投稿」「ループ予約」ボタンはすべて postingModal を開く（旧 loopModal を統合）。

- `mode: 'immediate' | 'scheduled' | 'loop'` の3モードをタブで切り替え
- `destinations: string[]` で投稿先をチェックボックスで選択（blog/news/instagram/instagram_story/threads）
- ループモード時のみループ設定表示
- `overrideDestinations` として `postToBlog` と `saveToSupabase` 両方に渡す

`blogSettings.destinations` はデフォルト値であり、投稿前にモーダルで per-post 上書きできる。

### 3サイズ画像生成（16:9・1:1・9:16）

記事生成時に `Promise.all` で3枚並行生成：

```javascript
const [imgResp16x9, imgResp1x1, imgResp9x16] = await Promise.all([
  ai.models.generateContent({ model: 'gemini-2.5-flash-image', ..., config: { imageConfig: { aspectRatio: "16:9" } } }),
  ai.models.generateContent({ model: 'gemini-2.5-flash-image', ..., config: { imageConfig: { aspectRatio: "1:1" } } }),
  ai.models.generateContent({ model: 'gemini-2.5-flash-image', ..., config: { imageConfig: { aspectRatio: "9:16" } } }),
]);
```

BlogPost型フィールド：
- `imageUrl` — 16:9（WordPress・Threads）
- `imageUrl1x1` — 1:1（Instagramフィード優先）
- `imageUrl9x16` — 9:16（Instagramストーリーズ優先）

自前画像アップロードモード（uploadモード）は1枚のみ（imageUrl1x1/imageUrl9x16はundefined）。

投稿時の画像割り当て：
- Instaフィード → 1:1優先（`uploadedImageUrl1x1 || uploadedImageUrl`）
- Stories → 9:16優先（`uploadedImageUrl9x16 || uploadedImageUrl1x1 || uploadedImageUrl`）

### Supabaseテーブル（scheduled_posts）の新カラム

```sql
image_url_1x1     text
image_base64_1x1  text
image_url_9x16    text
image_base64_9x16 text
```

保存時：data URIで持っている場合はbase64列に、URLで持っている場合はurl列に保存。

### hasInstaバグ修正

- 変更前：`!!(instaAccount || (blogSettings.instagramBusinessId && ...))` → story-onlyでも `hasInsta=true` になった
- 変更後：`!!(instaAccount || (dests.includes('instagram') && blogSettings.instagramBusinessId && ...))` → `dests.includes('instagram')` チェックを追加して修正
