...
// --- Publish timer (every 60s). Local/long-running server only — not used on Vercel. ---
...
// --- Express app factory (shared by local dev server and Vercel serverless entry) ---
  ...
  app.get("/api/scheduled-posts", async (req, res) => {
    ...
    try {
      const { data, error } = await supabase
        .from('scheduled_posts')
        .select('id, title, scheduled_at, status, published_at, error_message, loop_enabled, loop_interval_days, loop_count, post_to_wp, post_to_instagram, post_to_instagram_story, post_to_threads, post_to_hpb, wp_post_id, instagram_post_id, threads_post_id')
...
// No app.listen() and no startPublishTimer() here: scheduled posting is
// handled by a Supabase Cron Job invoking the process-scheduled-posts
// Edge Function directly, independent of this function.
const app = createApp();
export default app;
